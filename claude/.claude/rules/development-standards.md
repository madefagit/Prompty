...
## Rule 15: English and Polish engineering artifacts

- Code, comments, logs, commit messages, documentation: **English and Polish only**.
- User-facing UI strings and conversations: any language the user prefers.
- **All CLAUDE.md rules must be in English and Polish** Translate before adding.
- **Zero exceptions.**
...
## Rule 24: Mandatory action plan (`PlanWJM.md`)

- **Before starting any non-trivial task**, create a `PlanWJM.md` file in the repository root (or working directory) with the full action plan.
- Present the plan to the user and **wait for explicit approval** before executing any step.
- The user may request changes to the plan — reflect all modifications in `PlanWJM.md` before proceeding.
- **Plan format**: use a checklist (`- [ ]` / `- [x]`) so each step's completion is visible.
- **During execution**: mark each completed step as `- [x]` in `PlanWJM.md` immediately after finishing it.
...
- **After all steps are complete**: delete `PlanWJM.md` from the working directory. The plan is ephemeral — it exists only for the duration of the task.
...
- If the upgrade itself would be a major effort (e.g., .NET 6 → .NET 10, Angular 14 → Angular 21+), flag it to the user with an estimate before proceeding — but still perform it unless the user explicitly defers.
- **Zero exceptions.**

...
description: "Angular coding standards — PrimeNG UI, Reactive Forms (mandatory), Transloco i18n, OnPush strategy, NgRx, OIDC silent refresh, WCAG accessibility"
type: development
tags: ["angular", "typescript", "frontend", "rxjs", "primeng", "transloco", "reactive-forms"]
---
...
- **Forms**: Angular Reactive Forms exclusively — template-driven forms are forbidden
- **Internationalisation**: Transloco — all user-facing strings go through `transloco` pipe or `TranslocoService`
...
## Forms — Angular Reactive Forms (Mandatory)

- **Template-driven forms are forbidden.** All forms use `ReactiveFormsModule`.
- Build forms with `FormBuilder`, `FormGroup`, `FormControl`, `FormArray`.
- Validators: use built-in (`Validators.required`, `Validators.email`, etc.) and custom synchronous/async validators.
- Validation error messages: display conditionally on `control.invalid && control.touched`.
- Form state: use `form.valid`, `form.dirty`, `form.pristine` for submit button guards.
- Nested forms: use `FormGroup` inside `FormGroup` or `ControlValueAccessor` for reusable form components.

## Internationalisation — Transloco (Mandatory)

- All user-facing strings must use the `transloco` pipe or `TranslocoService.translate()`.
- **No hardcoded strings in templates or components** — zero exceptions.
- Translation files: `assets/i18n/{lang}.json` (e.g., `pl.json`, `en.json`).
- Default language: Polish (`pl`). Fallback language: English (`en`).
- Lazy-loaded scopes: each feature module loads its own translation scope.
- Typed translations: use `TranslocoKeys` type generation to catch missing keys at compile time.
- Dynamic values: use interpolation params (`{{ 'key' | transloco: { name: user.name } }}`).

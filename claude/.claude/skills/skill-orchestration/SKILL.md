...
description: "PlanWJM.md lifecycle, phase transition protocols, quality gate checklist, and post-task retrospective format for SDLC workflow"
type: workflow
...
## Plan Management (PlanWJM.md)

Before starting any non-trivial task:

1. **Create** `PlanWJM.md` in the repository root (or working directory) with the full action plan.
...
4. **Modifications**: the user may request changes — reflect all modifications in `PlanWJM.md` before proceeding.
...
7. **Cleanup**: after all steps are complete, delete `PlanWJM.md` from the working directory. The plan is ephemeral.
...
- Update `PlanWJM.md` after each step completion

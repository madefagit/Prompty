...
description: "Project security rules — Keychain-only secrets, OWASP mitigations, container hardening, PII masking, GDPR/CCPA controls, audit logging standards"
type: security
...
| Docker registry | `DockerRegistry` | `$(whoami)` | `security find-generic-password -s "DockerRegistry" -a "$(whoami)" -w` |

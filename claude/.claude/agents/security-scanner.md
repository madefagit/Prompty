...
description: "Mandatory SDLC gate (Phase 4.5) — scans code via taint analysis, produces CVSS-scored OWASP/CWE findings and orchestrator-ready remediation tasks"
type: agent
...
- **EF Core compliance**: flag any .NET code using raw ADO.NET or Dapper instead of EF Core 9 + Npgsql.
...
| `dotnet-developer` | Receives .NET-specific remediation tasks (EF Core query fixes, input validation, auth fixes). |
